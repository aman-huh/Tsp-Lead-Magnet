...
export default function ClientShowcase({
  ...
  const repeatCount =
    displayLogos.length > 0 ? Math.max(2, Math.ceil(10 / displayLogos.length)) : 1;
  const marqueeLogos =
    displayLogos.length > 0
      ? Array.from({ length: repeatCount }).flatMap(() => displayLogos)
      : [];

  return (
    <div className={`space-y-4 sm:space-y-5 mb-5 ${className}`}>
      {heading && (
        <p className="font-satoshi text-white font-[300] text-[clamp(0.6rem,3.3vw,1.375rem)] leading-snug tracking-[-0.01em] text-center lg:text-left">
          {heading}
        </p>
      )}
      ...
      <div className="relative w-full overflow-hidden">
        ...
        {marqueeLogos.length > 0 ? (
          ...
          <div className="flex items-center gap-8 sm:gap-10 lg:gap-12 text-white/80 font-satoshi text-base sm:text-lg py-1">
            <span>PALOMA</span>
            <span>figo</span>
            <span>SIORAI</span>
            <span>WESTSIDE</span>
